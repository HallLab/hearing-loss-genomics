...
sink(paste("allGenes/HL_", pheno, "/results_to_meta_degHL/", pheno, "_EUR_chr", chrom, ".txt", sep=""))
for (i in 15:length(eur)){
  print(summary(lm(Deg_HL ~ Sex + Age + AgeSq + PC1 + PC2 + PC3 + PC4 + eur[,i], data = eur)))
}
sink()

afr <- subset(merged, Ancestry == "AFR")
sink(paste("allGenes/HL_", pheno, "/results_to_meta_degHL/", pheno, "_AFR_chr", chrom, ".txt", sep=""))
for (i in 15:length(afr)){
  print(summary(lm(Deg_HL ~ Sex + Age + AgeSq + PC1 + PC2 + PC3 + PC4 + afr[,i], data = afr)))
}
sink()
