...
# Environment for plink 1.9 + biobin (hall_shared biobin module + liblzma shim).
# biobin/2.3.1 lives in the Hall tree, so this runs WITHOUT the `ritchie` group;
# the module pulls in boost 1.65.1 + the liblzma shim and puts biobin on PATH.
# shellcheck disable=SC1091
...
# LOKI: hall_shared copy, so this is self-contained — a `hall_shared` guest can
# run it without `hall`/`ritchie`. Daniel's loki-20220926 (2022-09 era) was
# removed from LPC; loki-20230816 is the closest available. biobin uses loki for
# gene/region metadata, but the boundaries we care about come from Phase 4's
# gene_list_regions.txt via --region-file, so the loki version shift minimally
# affects results.
...
which biobin >/dev/null       || fail "biobin not on PATH — 'module load biobin/2.3.1' failed (is /project/hall_shared/env/modulefiles on MODULEPATH? are you in the hall_shared group?)"
